    ...
    <nav className="navbar row">
        <div className="col-12 col-md-3">
          <div className="navbar-brand">
            <Link to="/">
              <img width="150px" alt='AURA Logo' src="/images/Auralogo.png" />
            </Link>
            </div>
        </div>
        ...
        <div className="col-12 col-md-3 mt-2 mt-md-0 d-flex align-items-center justify-content-end" style={{gap: '1rem'}}>
          <ThemeToggle/>
          { isAuthenticated ? 
            (
              <Dropdown className='d-inline' >
                  <Dropdown.Toggle variant='default pr-5' id='dropdown-basic' style={{background: 'transparent', border: 'none', color: 'inherit'}}>
                  ...
                  <Dropdown.Menu>
                      { user.role === 'admin' && <Dropdown.Item onClick={() => {navigate('admin/dashboard')}} className='dropdown-item-custom'><i className="fa fa-tachometer-alt"></i> Dashboard</Dropdown.Item> }
                      { user.role === 'merchant_admin' && <Dropdown.Item onClick={() => {navigate('/merchant/dashboard')}} className='dropdown-item-custom'><i className="fa fa-store"></i> Dashboard</Dropdown.Item> }
                      ...
                      <Dropdown.Item onClick={() => {navigate('/myprofile')}} className='dropdown-item-custom'><i className="fa fa-user"></i> Profile</Dropdown.Item>
                      { user.role === 'user' && <Dropdown.Item onClick={() => {navigate('/orders')}} className='dropdown-item-custom'><i className="fa fa-shopping-bag"></i> Orders</Dropdown.Item> }
